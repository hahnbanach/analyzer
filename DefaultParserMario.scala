...
/**
  ...
  */
abstract class DefaultParserMario(command: String, restrictedArgs: Map[String, String]) extends AbstractParser(command: String) {
  ...
  def gobbleCommands(commands: String): AbstractOperator = {
    ...
    def loop(chars: List[Char], index: Int, parenthesisBalance: List[Int], quoteBalance: Int,
      ...
      println("LOOP: ...........")
      println("DEBUG char " + chars(index))
      println("DEBUG parenthesisBalance " + parenthesisBalance)
      println("DEBUG quoteBalance " + quoteBalance)
      println("DEBUG commandBuffer " + commandBuffer)
      println("DEBUG argumentBuffer " + argumentBuffer)
      ...
      if (index >= chars.length && chars.nonEmpty) {
       ...
       //orig } else if (!justClosedParenthesis) ""
