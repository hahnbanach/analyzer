...
/** Product Operator
  ...
  */
class SumOperator(children: List[Expression]) extends AbstractOperator(children: List[Expression]) {
  override def toString: String = "SumOperator(" + children.mkString(", ") + ")"
  def add(e: Expression, level: Int = 0): AbstractOperator = {
    if (level === 0) new SumOperator(e :: children)
    else {
      children.headOption match {
        case Some(t) =>
          t match {
            case c: AbstractOperator => new SumOperator(c.add(e, level - 1) :: children.tail)
            case _ => throw OperatorException("SumOperator: trying to add to smt else than an operator")
          }
        case _ =>
          throw OperatorException("SumOperator: trying to add None instead of an operator")
      }
    }
  }
  ...
  def evaluate(query: String, data: AnalyzersData = AnalyzersData()): Result = {
    def operationFunction(l: List[Expression]): Result = {
      val valHead = l.headOption match {
        ...
        case _ => throw OperatorException("SumOperator: inner expression is empty")
      }
      if (l.tail.isEmpty) {
